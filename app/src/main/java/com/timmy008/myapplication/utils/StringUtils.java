...
public final class StringUtils {
        ...
        if ((object instanceof String) || (object instanceof Number) || (object instanceof Map) || (object instanceof Collection)) {
            ...
            try {
                for (Field field : fieldList) {
                    ...
                    sb.append(ReflectUtils.getFieldValue(object, field.getName())).append(',');
                }
                if (sb.length() > 0) {
     ...
     * @return
     */
    public static String getPercent(double num, int sacle) {
        NumberFormat nf = NumberFormat.getPercentInstance();
     ...
     * @return
     */
    public static float getStarNumber(float star) {
        float result;
        int score = (int) star;
        result = Math.abs(score - star);
    ...
    /**
     * 判断字符串是否为最多只有integerLength个整数位且只有scale位小数的数值
     *
     * @param number
     * @param scale
     * @return
     */
    public static boolean checkDecimal(String number, int integerLength, int scale) {
        if (integerLength < 1) {
            throw new IllegalArgumentException("The integerLength must be a positive integer!");
        } else if (scale < 1) {
            throw new IllegalArgumentException("The scale must be a positive integer!");
        }
        String regex = "^[0-9]{1,%d}(\\.[0-9]{0,%d})?$";
        regex = String.format(regex, integerLength, scale);
        Pattern pattern = Pattern.compile(regex);
        Matcher matcher = pattern.matcher(String.valueOf(number));
        return matcher.matches();
    }
    ...
    /**
     * 判断字符串是否为最多只有9个整数位且只有2位小数的数值
     *
     * @param number
     * @return
     */
    public static boolean checkInputCommDecimal(String number) {
        return checkDecimal(number, 9, 2);
    }

    /**
     * 判断字符串是否为最多只有4个整数位且只有2位小数的数值
     *
     * @param number
     * @return
     */
    public static boolean checkInputCashDecimal(String number) {
        return checkDecimal(number, 4, 2);
    }
